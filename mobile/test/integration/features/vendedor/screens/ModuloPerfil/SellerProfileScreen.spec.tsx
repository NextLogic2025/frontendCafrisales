...
const { SellerProfileScreen } = require('../../../../../../src/features/vendedor/screens/ModuloPerfil/SellerProfileScreen')

describe('integration/vendedor/SellerProfileScreen', () => {
  beforeEach(() => {
    jest.clearAllMocks()
    mockGetProfile.mockResolvedValue({ id: 's1', name: 'Seller', email: 's@c.com', phone: '1', role: 'seller' })
  ...
  it('loads profile', async () => {
    ...
    await waitFor(() => expect(screen.getByText('Seller')).toBeTruthy())
  })
  ...
  it('updates profile via callback', async () => {
    const screen = render(<SellerProfileScreen />)
    await waitFor(() => expect(screen.getByText('UpdateProfile')).toBeTruthy())

    await act(async () => fireEvent.press(screen.getByText('UpdateProfile')))

    await waitFor(() => expect(mockUpdateProfile).toHaveBeenCalledWith('s1', { nombre: 'Nuevo', telefono: '099' }))
